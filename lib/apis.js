...
function getDiffDataTable(olderData, newData) {
    ...
    for (let i = 0; i < olderData.length; i++) {
        for (let j = i; j < newData.length; j++) {
            if (olderData[i].uid === newData[j].uid) { // if same place
                difftable[j] = {
                    "pos": j,
                    "name": newData[j].name,
                    "diffpoint": (newData[j].points - olderData[j].points),
                    "point": newData[j].points
                }
            } else {
                for (let k = 0; k < newData.length; k++) { // not in sameplace find again
                    if (olderData[i].uid === newData[k].uid) {
                        difftable[k] = {
                            "pos": k,
                            "name": newData[k].name,
                            "diffpoint": (newData[k].points - olderData[k].points),
                            "point": newData[k].points
                        }
                    } else { // new record
                        difftable[j] = {
                            "pos": j,
